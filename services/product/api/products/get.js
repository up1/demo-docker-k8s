...
const GetProducts = {
  ...
  async listAllProducts(ctx) {
    try {
      if (ctx.query.search) {
        ctx.body = Object.assign({}, { products: await ProductService.getProductsBySearch(ctx.query.search) })
      } else {
        ctx.body = Object.assign({}, { products: await ProductService.getAllProducts() })
      }
      ctx.status = 200
    } catch (err) {
      logger.error(err)
